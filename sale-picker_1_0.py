...
def pick_sales_log(df_log, list_shop_name: list[str],
                   list_item: list[str], item_count: int):
...
def dropdown_shop_name(df_log):
    ...
    del df_shop["商品コード"]
    del df_shop["日付"]
    df_shop = df_shop.groupby("書店グループ名").agg(np.sum).reset_index()
...
if __name__ == "__main__":
    ...
    with gr.Blocks() as interface:
        ...
        data_frame_items = gr.DataFrame(
            headers=["No", "商品コード", "過去30日間売上", "コンテンツ名",
